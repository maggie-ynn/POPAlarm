...
fn main() -> Result<(), eframe::Error> {
    ...
    exe_dir.pop();
    let ini_path = exe_dir.join("conf.ini");
    ...
    let result = fs::read(icon_path.as_path());
    if let Ok(_) = result {
        icon_exist = true;
    }
    ...
    tray_menu.append_items(&[
        &PredefinedMenuItem::about(
            None,
            Some(AboutMetadata {
                name: Some("POPAlarm".to_string()),
                copyright: Some("POPAlarm".to_string()),
                ..Default::default()
            }),
        ),
    ...
    let options = eframe::NativeOptions {
        ...
        always_on_top: true,
        run_and_return: true,
        min_window_size: Some(egui::vec2(336.0, 104.0)),
        initial_window_size: Some(egui::vec2(336.0, 104.0)),
        default_theme: eframe::Theme::Dark,
        multisampling: 2,
        ..Default::default()
    };
    eframe::run_native(
        "POPAlarm", // unused title
        options,
        Box::new(move |_cc| {
            #[cfg(not(target_os = "linux"))]
            {
            ...
            Box::new(POPAlarm::new(
                ini_path.to_string_lossy().to_string(),
                quit_i.id(),
                settings_i.id(),
                time_str,
                sound_path,
